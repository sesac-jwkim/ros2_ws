...
class LaneDetector(object):
    ...
    def add_horizontal_line(self, image):
        ...
        roi_h_min = 0
        roi_h_max = h
        roi = image[roi_h_min:roi_h_max, roi_w_min:roi_w_max]  # 이미지의 오른쪽 절반만 자름
        flip_binary = cv2.flip(roi, 0)  # 상하 반전 (가장 위쪽의 픽셀을 쉽게 찾기 위해)
        max_y = cv2.minMaxLoc(flip_binary)[-1][1]  # 값이 255(흰색)인 가장 첫 번째 픽셀의 Y좌표 추출
    ...
    def add_vertical_line_far(self, image):
        ...
        flip_binary = cv2.flip(roi, -1)  # 상하좌우 반전시켜 탐색 시작점을 우측 하단으로 맞춤
    ...
    def __call__(self, image, result_image):
        ...
        for roi in self.rois:
            ...
            if max_contour_area is not None:
                ...
                cv2.drawContours(result_image, [box], -1, (255, 255, 0), 2)  # 화면에 사각형 박스 그리기
        ...
        for i in range(len(center_x)):
            if center_x[i] != -1:
                ...
                centroid_sum += center_x[i] * self.rois[i][-1] # 구역별 가중치를 곱해 더함
...
def main():
    ...
    while running:
        ...
        cv2.fillPoly(binary_image, [np.array(roi)], [0, 0, 0])  # y 좌표 윗부분(먼 곳의 노이즈)은 흑백으로 칠해 간섭 차단
        min_x = cv2.minMaxLoc(binary_image)[-1][0]
        cv2.line(img, (min_x, y), (640, y), (255, 255, 255), 50)  # 조향을 돕기 위한 굵은 가상선(수평선) 렌더링
